...
export const StoreApp: React.FC = () => {
  ...
  const [zinliEmail, setZinliEmail] = useState(store.metodosPago?.zinli?.email || '');
  const [binancePayId, setBinancePayId] = useState(store.metodosPago?.binance?.payId || '');
  const [binanceNick, setBinanceNick] = useState(store.metodosPago?.binance?.nickname || '');
  const [paypalEmail, setPaypalEmail] = useState(store.metodosPago?.paypal?.email || '');
  ...
  const totalVentasUsd = storeOrders
    ...
    .reduce((sum, o) => sum + (o.montoSubtotalUsd || 0), 0);
  ...
  const handleStoreLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStoreLoginError('Error de comunicación con la base de datos.');
    } finally {
  ...
  const allCategories = ['todas', ...Array.from(new Set(storeProducts.map(p => p?.categoria || 'General')))];
  const filteredProducts = storeProducts
    .filter(p => {
      ...
      const matchSearch = term === '' || pNom.includes(term) || pDesc.includes(term);
  ...
  return (
    <div className="flex flex-col h-full bg-neutral-50 dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
      ...
      {/* Demo Store Selector Bar (Solo si hay múltiples comercios disponibles) */}
      {stores.length > 1 && (
                      ...
                      <div className="p-2.5 rounded-xl bg-neutral-100 dark:bg-neutral-800 flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2 min-w-0">
                          ...
                          <div className="text-[11px] min-w-0">
                            ...
                            <p className="text-[10px] text-neutral-400 font-mono truncate">Placa: {order.conductor?.moto?.placa || 'S/P'}</p>
                          </div>
                        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-xs p-3">
          <div className="w-full max-w-sm bg-white dark:bg-neutral-900 rounded-3xl p-4 border border-neutral-700 shadow-2xl text-xs space-y-3">
            ...
            <div className="p-2.5 rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-[11px] space-y-1 text-emerald-800 dark:text-emerald-300">
              ...
              <p>✓ Licencia Grado 2da (Motos): {selectedOrderForDriverModal.conductor?.legal?.licenciaNumero || 'En regla'}</p>
              <p>✓ Certificado Médico Vial MPPS: Vence {selectedOrderForDriverModal.conductor?.legal?.certificadoMedicoVencimiento || '2026-12-31'}</p>
              <p>✓ RCV: {selectedOrderForDriverModal.conductor?.legal?.rcvAseguradora || 'Seguros Caracas'}</p>
            </div>
