...
export const StoresManager: React.FC = () => {
  ...
  const filteredStores = useMemo(() => {
    return stores.filter(s => {
      ...
      const sNom = (s.nombre || '').toLowerCase();
      const sCat = (s.categoria || '').toLowerCase();
      const sRif = (s.rif || '').toLowerCase();
      const sDir = (s.direccion || '').toLowerCase();
      const matchesSearch = !q || sNom.includes(q) || sCat.includes(q) || sRif.includes(q) || sDir.includes(q);
    ...
    <div className="space-y-5">
      ...
      <div className="space-y-3">
        {filteredStores.length === 0 ? (
          <div className="p-10 text-center bg-white dark:bg-neutral-900 rounded-2xl border border-neutral-200 dark:border-neutral-800 text-neutral-400 space-y-3">
            <Store className="w-10 h-10 mx-auto text-neutral-300 dark:text-neutral-700" />
            {stores.length === 0 ? (
              <>
                <p className="text-sm font-semibold text-neutral-600 dark:text-neutral-300">Aún no hay comercios afiliados</p>
                <p className="text-xs text-neutral-400 max-w-xs mx-auto">
                  Los comercios que se registren en <strong>/registro-comercios/</strong> aparecerán aquí para su aprobación y gestión.
                </p>
                <button
                  onClick={() => refreshBackendData()}
                  className="inline-flex items-center gap-1.5 text-xs text-purple-600 hover:underline font-bold cursor-pointer"
                >
              ...
              <>
                <p className="text-sm font-medium">No hay comercios en el rubro o búsqueda seleccionada.</p>
                <button 
