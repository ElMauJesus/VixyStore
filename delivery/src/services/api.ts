...
class ApiService {
  ...
  // --- COMERCIOS ---
  public async getComercios(params: { categoria?: string; solo_activos?: boolean } = {}) {
    const q = new URLSearchParams();
    if (params.categoria && params.categoria !== 'todas') q.set('categoria', params.categoria);
    if (params.solo_activos) q.set('solo_activos', '1');
    const qs = q.toString() ? `?${q.toString()}` : '';
    return this.request<{ success: boolean; total: number; comercios: any[] }>(`/comercios.php${qs}`);
  }

  public async getComercio(id: string) {
    return this.request<{ success: boolean; comercio: any }>(`/comercios.php?id=${encodeURIComponent(id)}`);
  }

  public async updateComercio(id: string, data: any, action?: string) {
    const act = action ? `&action=${encodeURIComponent(action)}` : '';
    return this.request<{ success: boolean; mensaje: string }>(`/comercios.php?id=${encodeURIComponent(id)}${act}`, {
      method: 'PUT',
  ...
  public async getReclamos(estado?: string) {
    ...
    return this.request<{ success: boolean; reclamos: any[] }>(`/reclamos.php${q}`);
  }
