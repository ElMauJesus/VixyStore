...
if ($method === 'POST') {
    ...
    $gananciaApp = round($costoDelivery * 0.15, 2);
    $gananciaConductor = round($costoDelivery - $gananciaApp, 2);

    $totalUsd = $subtotal + $costoDelivery;
    $totalBs = $totalUsd * $tasaBcv;

    $stmt = $pdo->prepare("
        INSERT INTO pedidos (
            id, codigo_seguimiento, cliente_id, comercio_id, estado,
            monto_subtotal_usd, costo_producto_usd, costo_envio_usd, costo_delivery_usd,
            ganancia_conductor_usd, ganancia_app_usd,
            tasa_bcv_bs, monto_total_usd, monto_total_bs,
    ...
    Database::jsonResponse([
        ...
        'costo_producto_usd' => $subtotal,
        'costo_delivery_usd' => $costoDelivery,
        'ganancia_conductor_usd' => $gananciaConductor,
        'ganancia_app_usd' => $gananciaApp,
        'costo_envio_usd' => $costoDelivery,
        'total_usd' => $totalUsd,
        'total_bs' => $totalBs
    ], 201);
}
