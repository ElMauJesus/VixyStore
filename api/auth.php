...
if ($action === 'login' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($appRole === '' || $appRole === 'cliente') {
        $stmtClient = $pdo->prepare("SELECT id, nombre, apellido, cedula, email, telefono, password_hash, direccion_habitual, activo FROM clientes WHERE email = :id1 OR email = :generated_email OR telefono = :id2 OR cedula = :id3 OR REPLACE(REPLACE(telefono, '-', ''), ' ', '') = :telefono_compacto OR REPLACE(REPLACE(cedula, '-', ''), ' ', '') = :cedula_compacta LIMIT 1");
        $stmtClient->execute(['id1' => $identifier, 'generated_email' => $identifierEmail, 'id2' => $identifier, 'id3' => $identifier, 'telefono_compacto' => $identifierCompact, 'cedula_compacta' => strtoupper($identifierCompact)]);
    ...
    if ($appRole === '' || $appRole === 'conductor') {
        try {
            $driverColumns = array_column($pdo->query('SHOW COLUMNS FROM conductores')->fetchAll(), 'Field');
            $requiredDriverColumns = ['id', 'nombre', 'apellido', 'email', 'telefono', 'cedula', 'password_hash'];
            $missingDriverColumns = array_values(array_diff($requiredDriverColumns, $driverColumns));
            if ($missingDriverColumns) {
                throw new RuntimeException('Faltan columnas base: ' . implode(', ', $missingDriverColumns));
            }

            $driverSelect = [];
            foreach (['id', 'nombre', 'apellido', 'email', 'telefono', 'cedula', 'password_hash'] as $column) {
                $driverSelect[] = '`' . $column . '`';
            }
            $optionalDriverColumns = [
                'foto_url' => "''",
                'disponible' => '0',
                'saldo_billetera_usd' => '0',
                'bloqueado_por_saldo' => '0',
                'verificado_por_admin' => '0'
            ];
            foreach ($optionalDriverColumns as $column => $fallback) {
                $driverSelect[] = in_array($column, $driverColumns, true)
                    ? '`' . $column . '`'
                    : $fallback . ' AS `' . $column . '`';
            }

            $driverWhere = ['`email` = :id1', '`telefono` = :id2', '`cedula` = :id3'];
            if (in_array('telefono', $driverColumns, true)) {
                $driverWhere[] = "REPLACE(REPLACE(`telefono`, '-', ''), ' ', '') = :telefono_compacto";
            }
            if (in_array('cedula', $driverColumns, true)) {
                $driverWhere[] = "REPLACE(REPLACE(`cedula`, '-', ''), ' ', '') = :cedula_compacta";
                $driverWhere[] = '`cedula` = :cedula_normalizada';
            }
            $stmtDriver = $pdo->prepare('SELECT ' . implode(', ', $driverSelect) . ' FROM conductores WHERE ' . implode(' OR ', $driverWhere) . ' LIMIT 1');
            $stmtDriver->execute([
                'id1' => $identifier,
                'id2' => $identifier,
                'id3' => $identifier,
                'telefono_compacto' => $identifierCompact,
                'cedula_compacta' => strtoupper($identifierCompact),
                'cedula_normalizada' => $driverCedula
            ]);
            $driver = $stmtDriver->fetch();
        } catch (Throwable $error) {
            error_log('Vixy login conductor, estructura de conductores: ' . $error->getMessage());
            Database::jsonResponse([
                'error' => true,
                'mensaje' => 'La tabla conductores no tiene las columnas base necesarias. Ejecute la migración en c2861522_vixy_dl. Detalle: ' . $error->getMessage()
            ], 500);
        }
    }
    ...
    if ($driver) {
        ...
        if ($validDriverPass) {
            ...
            $isBlocked = ($saldoUsd <= -0.50) || (bool)$driver['bloqueado_por_saldo'];
            ...
            try {
                ...
                Database::jsonResponse([
                    'error' => true,
                    'mensaje' => 'La cuenta fue validada, pero el servidor no pudo crear la sesión. Ejecute sql/00_MIGRACION_UNIFICADA_ULTIMOS_CAMBIOS.sql en la base de datos operativa.'
                ], 500);
            }
            Database::jsonResponse([
                'success' => true,
