...
$cedulaReferencia = trim((string)($data['cedula'] ?? ''));
...
try {
    ...
    if ($tipoUsuario === 'conductor') {
        $idDigits = preg_replace('/[^0-9]/', '', (string)$usuarioId);
        if ($latitud != 0 && $longitud != 0) {
            $stmtSyncCond = $pdo->prepare("
                ...
                SET latitud_actual = :lat,
                    ...
                    ultima_actualizacion = NOW()
                     WHERE id = :uid_id
                         OR codigo_conductor = :uid_code
                         OR cedula = :uid_cedula
                         OR (:cedula_guard != '' AND cedula = :cedula_value)
            ...
            $stmtSyncCond->execute([
                ...
                'uid_id' => $usuarioId,
                'uid_code' => $usuarioId,
                'uid_cedula' => $usuarioId,
                'cedula_guard' => $cedulaReferencia,
                'cedula_value' => $cedulaReferencia,
                'digits_guard' => $idDigits,
                'digits_value' => $idDigits
            ]);
        } elseif ($online === 0) {
            // Remove the last coordinate when the app cannot provide a fresh GPS fix.
            $stmtSyncOffline = $pdo->prepare("
                UPDATE conductores
                SET disponible = 0,
                    latitud_actual = NULL,
            ...
            // Sin GPS real aún: solo refrescar presencia (disponible)
