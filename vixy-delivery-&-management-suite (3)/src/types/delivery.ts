...
export interface Conductor {
  ...
  estadoRegistro?: 'aprobado' | 'pendiente_aprobacion' | 'rechazado';
  documentosFotos?: {
    cedulaFotoUrl?: string;
    licenciaFotoUrl?: string;
    certificadoMedicoFotoUrl?: string;
    carnetCirculacionFotoUrl?: string;
...
export interface TarifasDeliveryConfig {
  tasaBcvBs: number;
  porcentajeComisionDelivery: number; // Ej: 15% comisión de empresa por carrera delivery
  porcentajeComisionComercio?: number; // Ej: 15% comisión de empresa sobre ventas comerciales
...
export interface LiquidacionComercio {
  id: string;
  comercioId: string;
  comercioNombre: string;
  ...
  metodoPago: MetodoPagoTipo;
  bancoDestino?: string;
  cuentaTelefonoDestino?: string;
  referenciaBancaria: string;
  comprobanteUrl?: string;
  comprobanteRutaSql: string;
  estado: 'procesado' | 'en_verificacion';
  autorizadoPor: string;
  notas?: string;
  fecha: string;
}
...
export interface SolicitudRegistroConductor {
  id: string;
  nombre: string;
  apellido: string;
  cedula: string;
...
export interface CarterasConfig {
  habilitadasGlobal: boolean;
  comerciosHabilitada: boolean;
  deliveriesHabilitada: boolean;
  clientesHabilitada: boolean;
  mensajeMantenimiento?: string;
}

export interface PagoMovilEmpresaConfig {
  banco: string;
  telefono: string;
  cedulaRif: string;
