...
export const StoreApp: React.FC = () => {
  ...
  const handleStoreLogout = (e?: React.MouseEvent) => {
  ...
  return (
    <div className="flex flex-col h-full bg-neutral-50 dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
      ...
      <div className="px-3 py-2.5 bg-white dark:bg-neutral-850 border-b border-neutral-200 dark:border-neutral-800 flex items-center justify-between shrink-0 gap-2">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          ...
          <button
            ...
            className="px-2 py-1 rounded-lg bg-red-500/10 hover:bg-red-500/20 text-red-600 dark:text-red-400 border border-red-500/30 transition cursor-pointer flex items-center gap-1 text-[10px] font-bold"
      ...
      <div className="flex-1 min-h-0 overflow-y-auto p-3 space-y-3">
          ...
          <div className="p-3 bg-gradient-to-r from-amber-500 via-purple-600 to-amber-600 text-white rounded-2xl shadow-md space-y-2 shrink-0 border border-white/20">
            <div className="flex items-center justify-between gap-2">
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    ...
                    <span className={`text-[10px] font-mono font-black px-1.5 py-0.2 rounded ${
                      (pendingApprovalOrders[0].tiempoRestanteAceptarComercio ?? 60) <= 15
                        ? 'bg-red-600 text-white animate-pulse'
                        : 'bg-white/30 text-white'
                    }`}>
                      ⏳ {Math.floor((pendingApprovalOrders[0].tiempoRestanteAceptarComercio ?? 60) / 60)}:
                      {((pendingApprovalOrders[0].tiempoRestanteAceptarComercio ?? 60) % 60) < 10 ? '0' : ''}
                      {(pendingApprovalOrders[0].tiempoRestanteAceptarComercio ?? 60) % 60}s
                    </span>
                  </div>
              ...
              <div className="flex items-center gap-1 shrink-0">
                <button
                  type="button"
                  onClick={() => playBellSound('store')}
                  title="Campana de aviso"
                  className="p-1 bg-white/20 hover:bg-white/30 rounded-lg text-white cursor-pointer transition"
                >
                  <Volume2 className="w-3.5 h-3.5" />
                </button>
                <span className="text-[10px] font-mono bg-white/20 px-2 py-0.5 rounded-lg">
                  {pendingApprovalOrders[0].metodoPagoSeleccionado === 'saldo_cartera' ? 'Cartera' : (pendingApprovalOrders[0].metodoPagoSeleccionado ? pendingApprovalOrders[0].metodoPagoSeleccionado.replace('_', ' ') : 'Pago')}
                </span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-1.5 pt-0.5">
              ...
              >
                ...
                <span>Aceptar ({pendingApprovalOrders[0].tiempoRestanteAceptarComercio ?? 60}s)</span>
              </button>
              <button
                ...
                <div className="space-y-3 pt-1">
                  {pendingApprovalOrders.map(order => (
                    ...
                    >
                      ...
                      {/* 1-Minute Acceptance Timer & Sound Notification Alert Banner */}
                      <div className="p-2 bg-amber-500/10 dark:bg-amber-500/20 border border-amber-500/30 rounded-xl flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <span className="relative flex h-2.5 w-2.5">
                            <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-amber-400 opacity-75"></span>
                            <span className="relative inline-flex rounded-full h-2.5 w-2.5 bg-amber-500"></span>
                          </span>
                          <span className="text-[11px] font-extrabold text-amber-700 dark:text-amber-300 flex items-center gap-1">
                        ...
                        <div className="flex items-center gap-2">
                          <span className={`font-mono font-black text-xs px-2 py-0.5 rounded-md ${
                            (order.tiempoRestanteAceptarComercio ?? 60) <= 15
                              ? 'bg-red-600 text-white animate-pulse'
                              : 'bg-amber-500 text-white'
                          }`}>
                            00:{(order.tiempoRestanteAceptarComercio ?? 60) < 10 ? '0' : ''}{order.tiempoRestanteAceptarComercio ?? 60}s
                          </span>
                          <button
                            type="button"
                            onClick={() => playBellSound('store')}
                            ...
                            <>
                              ...
                              <div>
                                <span className="font-bold text-blue-600 dark:text-blue-400 block capitalize">
                                  {order.metodoPagoSeleccionado ? order.metodoPagoSeleccionado.replace('_', ' ') : 'Pago Digital'}
                                </span>
                                <span className="text-[10px] text-neutral-400 font-mono">
                                  Ref: {order.referenciaPago || 'Sin Referencia'}
                                </span>
                      ...
                      <div className="p-2.5 rounded-xl bg-amber-500/10 border border-amber-500/30 text-xs space-y-2">
                        <div className="flex items-center justify-between">
                          <span className="font-bold text-amber-700 dark:text-amber-300">
                            Confirmar Pedido ({order.metodoPagoSeleccionado === 'saldo_cartera' ? 'Cartera Vixy' : (order.metodoPagoSeleccionado ? order.metodoPagoSeleccionado.replace('_', ' ') : 'Pago')})
                          </span>
                ...
                >
                  ...
                  <div className="text-right">
                    ...
                    <span className="text-[10px] text-neutral-400 capitalize">
                      {o.metodoPago ? o.metodoPago.replace('_', ' ') : 'Pago'}
                    </span>
                  </div>
                </div>
              ))}
        ...
        {activeTab === 'cartera' && (
          <div className="space-y-3.5 text-xs">
            ...
            {(!carterasConfig?.comerciosHabilitada || !carterasConfig?.habilitadasGlobal) && (
              <div className="p-3.5 bg-red-500/10 border border-red-500/30 rounded-2xl flex items-start gap-2.5 text-red-700 dark:text-red-400 text-xs">
                <AlertTriangle className="w-4 h-4 shrink-0 text-red-500 mt-0.5" />
                <div className="space-y-0.5">
                  <span className="font-bold block">Cartera Comercial Pausada desde Panel Central</span>
                  <p className="text-[11px] opacity-90">
                    {carterasConfig.mensajeMantenimiento || 'Las operaciones de retiro y movimientos en cartera están pausadas temporalmente por mantenimiento. Los cobros directos de tus pedidos continúan funcionando con normalidad.'}
                  </p>
                </div>
              </div>
            )}
            ...
            {/* Comisión de Empresa Card (Inicia en 0% - Precios no afectados) */}
            <div className="p-3.5 bg-white dark:bg-neutral-850 rounded-2xl border border-neutral-200 dark:border-neutral-800 flex items-center justify-between gap-3 shadow-2xs">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <span className="font-extrabold text-neutral-800 dark:text-neutral-200 text-xs">
                      Comisión de Servicios de Empresa:
                    </span>
                    <span className="px-2 py-0.5 rounded-full bg-amber-500 text-white font-mono font-black text-[11px]">
                      {porcentajeComisionComercio}%
                    </span>
                  </div>
                  <p className="text-[10px] text-neutral-400 mt-0.5">
                    {porcentajeComisionComercio === 0 
                      ? 'Tarifa 0%: No se descuenta comisión sobre tus ventas de catálogo.' 
                      : `Deducción del ${porcentajeComisionComercio}% aplicada en liquidación contable de cartera.`
                    } Los precios al cliente no están afectados.
                  </p>
                </div>
              </div>
              ...
              <button
                ...
                className="px-2.5 py-1.5 rounded-xl bg-neutral-100 dark:bg-neutral-800 hover:bg-neutral-200 dark:hover:bg-neutral-700 text-neutral-700 dark:text-neutral-300 font-bold text-[10px] flex items-center gap-1.5 transition cursor-pointer shrink-0 border border-neutral-200 dark:border-neutral-700"
              >
                <QrCode className="w-3.5 h-3.5 text-amber-500" />
                <span>Pago Móvil Oficial</span>
              </button>
            </div>
            ...
            <div className="p-3 bg-white dark:bg-neutral-850 rounded-2xl border border-neutral-200 dark:border-neutral-800 flex items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-neutral-400">Acreditaciones directas sincronizadas y gestionadas por ID de comercio</p>
                </div>
              </div>
              <span className={`text-[10px] font-mono px-2 py-0.5 rounded font-bold ${
                ...
                {(carterasConfig?.comerciosHabilitada && carterasConfig?.habilitadasGlobal) ? 'Sincronizada' : 'Pausada'}
              </span>
            </div>
      ...
      {/* Bottom Store Tabs */}
      <div className="sticky bottom-0 z-30 p-1.5 bg-white dark:bg-neutral-850 border-t border-neutral-200 dark:border-neutral-800 grid grid-cols-6 gap-0.5 shrink-0 text-center shadow-lg">
        <button
      ...
      <PagoMovilModal
        ...
        montoUsd={storeWallet?.saldoUsd || 0}
        concepto={`Gestión de Pago Móvil Oficial - ${store?.nombre || 'Vixy Store'}`}
      />
    </div>
  );
};
