...
export const ClientApp: React.FC = () => {
  ...
  const [metodoPago, setMetodoPago] = useState<MetodoPagoTipo>('pago_movil');
  ...
  // Authentication state for Vixy Pedidos (Default to registration screen as requested)
  ...
  const [regFotoCedula, setRegFotoCedula] = useState<string>('');
  const [regFotoPerfil, setRegFotoPerfil] = useState<string>('');
    ...
    return (
      <div className="flex flex-col h-full bg-neutral-100 dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100 p-4 items-center justify-center overflow-y-auto">
        <div className="w-full max-w-sm bg-white dark:bg-neutral-850 p-6 rounded-3xl border border-neutral-200 dark:border-neutral-800 shadow-xl space-y-4 my-auto">
          ...
          <DevicePermissionsBanner appName="Vixy Pedidos" />
            ...
            <form onSubmit={handleRegisterSubmit} className="space-y-2.5">
              ...
              <div className="space-y-1">
                    ...
                    onClick={() => {
                      if ('geolocation' in navigator) {
                        navigator.geolocation.getCurrentPosition(
                          (pos) => {
                            setRegDireccion(`Lat: ${pos.coords.latitude.toFixed(5)}, Lng: ${pos.coords.longitude.toFixed(5)} (Ubicación GPS actual)`);
                          },
                ...
                <input
                  ...
                  placeholder="Av, Edificio, Casa, Sector o GPS..."
                  className="w-full p-2 bg-neutral-50 dark:bg-neutral-800 rounded-xl border border-neutral-300 dark:border-neutral-700 text-xs"
                />
              </div>
              ...
              {/* Acceso a Cámara y Carga de Cédula y Selfie para Cliente */}
              <div className="space-y-1.5 pt-1">
                <label className="text-[10px] uppercase font-bold text-neutral-500 flex items-center gap-1">
                  <Camera className="w-3.5 h-3.5 text-amber-500" />
                  <span>Verificación Visual con Cámara</span>
                </label>
                <div className="grid grid-cols-2 gap-2">
                  <div className="p-2 bg-neutral-50 dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700 space-y-1">
                    <span className="text-[10px] font-bold text-neutral-700 dark:text-neutral-300 block truncate">Foto Cédula</span>
                    {regFotoCedula ? (
                      <div className="w-full h-12 rounded-lg overflow-hidden bg-neutral-200">
                        <img src={regFotoCedula} alt="Cédula" className="w-full h-full object-cover" />
                      </div>
                    ) : (
                      <div className="w-full h-12 rounded-lg border border-dashed border-neutral-300 dark:border-neutral-600 flex items-center justify-center text-[9px] text-neutral-400">
                        Sin foto
                      </div>
                    )}
                    <label className="w-full py-1 bg-neutral-200 dark:bg-neutral-700 hover:bg-amber-500 hover:text-white rounded-lg text-[9px] font-bold flex items-center justify-center gap-1 cursor-pointer transition">
                      <Camera className="w-2.5 h-2.5" />
                      <span>{regFotoCedula ? 'Cambiar' : 'Cámara'}</span>
                      <input 
                        type="file" 
                        accept="image/*" 
                        capture="environment"
                        ...
                        onChange={(e) => {
                          ...
                          if (file) {
                            ...
                            r.onloadend = () => { if (typeof r.result === 'string') setRegFotoCedula(r.result); };
                  ...
                  <div className="p-2 bg-neutral-50 dark:bg-neutral-800 rounded-xl border border-neutral-200 dark:border-neutral-700 space-y-1">
                    <span className="text-[10px] font-bold text-neutral-700 dark:text-neutral-300 block truncate">Foto Perfil/Selfie</span>
                    {regFotoPerfil ? (
                      <div className="w-full h-12 rounded-lg overflow-hidden bg-neutral-200">
                        <img src={regFotoPerfil} alt="Perfil" className="w-full h-full object-cover" />
                      </div>
                    ) : (
                      <div className="w-full h-12 rounded-lg border border-dashed border-neutral-300 dark:border-neutral-600 flex items-center justify-center text-[9px] text-neutral-400">
                        Sin foto
                    ...
                    <label className="w-full py-1 bg-neutral-200 dark:bg-neutral-700 hover:bg-amber-500 hover:text-white rounded-lg text-[9px] font-bold flex items-center justify-center gap-1 cursor-pointer transition">
                      <Camera className="w-2.5 h-2.5" />
                      <span>{regFotoPerfil ? 'Cambiar' : 'Cámara'}</span>
                      <input 
                        type="file" 
                        accept="image/*" 
                        capture="user"
                        ...
                        onChange={(e) => {
                          ...
                          if (file) {
                            const r = new FileReader();
                            r.onloadend = () => { if (typeof r.result === 'string') setRegFotoPerfil(r.result); };
                            r.readAsDataURL(file);
                          }
                        }}
                      />
  ...
  return (
    <div className="flex flex-col h-full bg-neutral-50 dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
      {/* Top Client Bar */}
      <div className="px-3 py-2.5 bg-white dark:bg-neutral-850 border-b border-neutral-200 dark:border-neutral-800 flex items-center justify-between gap-2 shrink-0">
        <div className="flex items-center gap-2.5 min-w-0 flex-1">
          ...
          <div className="min-w-0 flex-1 flex flex-col justify-center">
            <span className="text-xs font-bold text-neutral-900 dark:text-white truncate block leading-normal">
              {client.nombre} {client.apellido}
            </span>
            <div className="flex items-center gap-1.5 min-w-0 mt-0.5 leading-none">
              <span className="text-[9px] text-neutral-400 font-mono bg-neutral-100 dark:bg-neutral-800 px-1 py-0.5 rounded shrink-0">
                {client.cedula}
              </span>
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          <button
            ...
            className="px-2 py-1.5 rounded-xl bg-amber-500/10 hover:bg-amber-500/20 text-amber-600 dark:text-amber-400 text-[11px] font-mono font-bold flex items-center gap-1 border border-amber-500/30 transition cursor-pointer whitespace-nowrap"
          ...
          <button
            ...
            className="px-2 py-1.5 rounded-xl bg-red-500/10 hover:bg-red-500/20 text-red-600 dark:text-red-400 font-bold border border-red-500/30 transition cursor-pointer flex items-center gap-1 text-[10px]"
            ...
            {showCheckout && (
              <div className="bg-white dark:bg-neutral-850 p-4 rounded-2xl border border-amber-500/40 shadow-lg space-y-3">
                ...
                <div className="space-y-1.5">
                  ...
                  {(() => {
                    const carteraActiva = (carterasConfig?.habilitadasGlobal ?? true) && (carterasConfig?.clientesHabilitada ?? true);
                    return (
                      <div>
                        <button
                          type="button"
                          disabled={!carteraActiva}
                          onClick={() => carteraActiva && setMetodoPago('saldo_cartera')}
                          className={`w-full p-2.5 rounded-2xl text-left border transition flex items-center justify-between ${
                            !carteraActiva
                              ? 'opacity-60 bg-neutral-100 dark:bg-neutral-800 border-dashed border-neutral-300 dark:border-neutral-700 cursor-not-allowed'
                              : metodoPago === 'saldo_cartera'
                              ? 'border-amber-500 bg-amber-500/10 text-neutral-900 dark:text-white shadow-xs cursor-pointer'
                              : 'border-neutral-200 dark:border-neutral-700 text-neutral-600 dark:text-neutral-400 cursor-pointer'
                          }`}
                        >
                          <div className="flex items-center gap-2">
                            <div className={`w-8 h-8 rounded-xl flex items-center justify-center font-bold shrink-0 ${
                              carteraActiva ? 'bg-amber-500 text-white' : 'bg-neutral-400 text-white'
                            }`}>
                              <Wallet className="w-4 h-4" />
                            </div>
                            <div>
                              <span className="font-bold text-xs block">
                                Mi Cartera Vixy {carteraActiva ? '(Acreditación Inmediata)' : '(Desactivada por Panel Web)'}
                              </span>
                              <span className="text-[10px] text-neutral-500">
                                {carteraActiva 
                                  ? `Saldo actual: $${(clientWallet?.saldoUsd ?? 0).toFixed(2)} USD (Bs. ${((clientWallet?.saldoUsd ?? 0) * tasaBcv).toFixed(2)})`
                                  : (carterasConfig?.mensajeMantenimiento || 'Cartera en mantenimiento programado.')
                                }
                              </span>
                            </div>
                          </div>
                          {carteraActiva ? (
                            ...
                            <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-neutral-200 dark:bg-neutral-700 text-neutral-500">
                              Pausada
                            </span>
                          )}
                        </button>
                  ...
                  <div className="grid grid-cols-3 gap-1.5 text-[11px] pt-1">
                    <button
                      ...
                      className={`p-2 rounded-xl text-center border font-semibold transition cursor-pointer flex items-center justify-center gap-1 ${
                ...
                {metodoPago === 'pago_movil' && (
                  <div className="p-3 rounded-2xl bg-amber-500/5 dark:bg-amber-500/10 border border-amber-500/20 text-xs space-y-2.5">
                    <div className="flex items-center justify-between">
                      <span className="font-extrabold text-amber-700 dark:text-amber-400 flex items-center gap-1.5">
                        <QrCode className="w-4 h-4 text-amber-500" />
                        Pago Móvil Fijo Oficial Vixy
                      </span>
                      <button
                        type="button"
                        onClick={() => setShowPagoMovilModal(true)}
                        className="px-2.5 py-1 bg-amber-500 text-white rounded-xl font-bold text-[10px] flex items-center gap-1 hover:bg-amber-600 transition cursor-pointer shadow-2xs"
                      >
                        <QrCode className="w-3 h-3" />
                        <span>Ver QR & Copiar Datos</span>
                      </button>
                    </div>

                    <div className="bg-white dark:bg-neutral-800 p-2.5 rounded-xl border border-neutral-200 dark:border-neutral-700 text-[11px] space-y-1">
                      <div className="flex justify-between">
                        <span className="text-neutral-400">Banco:</span>
                        <span className="font-bold text-neutral-800 dark:text-neutral-200">{pagoMovilConfig.banco}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-neutral-400">Teléfono:</span>
                        <span className="font-mono font-bold text-neutral-800 dark:text-neutral-200">{pagoMovilConfig.telefono}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-neutral-400">RIF / C.I.:</span>
                        <span className="font-mono font-bold text-neutral-800 dark:text-neutral-200">{pagoMovilConfig.cedulaRif}</span>
                      </div>
                      <div className="flex justify-between">
                        <span className="text-neutral-400">Titular:</span>
                        <span className="font-bold text-neutral-800 dark:text-neutral-200">{pagoMovilConfig.nombreTitular}</span>
                      </div>
                      <div className="flex justify-between pt-1 border-t border-neutral-100 dark:border-neutral-700">
                        <span className="font-bold text-amber-600 dark:text-amber-400">Monto Exacto:</span>
                        <span className="font-mono font-black text-neutral-900 dark:text-white">Bs. {totalBs.toFixed(2)} (${totalUsd.toFixed(2)} USD)</span>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        ...
                        placeholder="Ingresa los dígitos de la referencia de pago"
                        className="w-full p-2 bg-white dark:bg-neutral-900 rounded-xl border border-neutral-300 dark:border-neutral-700 text-xs font-mono text-neutral-900 dark:text-white focus:outline-none focus:border-amber-500"
                      />
                    </div>
                  </div>
                )}
      ...
      {/* Modal Emergente Pequeño con QR y Datos Bancarios Oficiales */}
      <PagoMovilModal
        isOpen={showPagoMovilModal}
        onClose={() => setShowPagoMovilModal(false)}
        montoUsd={totalUsd}
        montoBs={totalBs}
        concepto={`Pedido en ${store?.nombre || 'Vixy Store'}`}
        initialReferencia={referencia}
        onConfirmReference={(ref) => setReferencia(ref)}
      />
    </div>
  );
};
