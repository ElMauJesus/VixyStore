...
import { DevicePermissionsBanner } from '../common/DevicePermissionsBanner';
...
export const DriverApp: React.FC = () => {
  ...
  const pendingOffers = orders.filter(o => {
    // 1. Si este conductor ya rechazó este servicio, debe desaparecer inmediatamente de su vista
    if (o.conductoresRechazaron?.includes(driver.id)) return false;

    // 2. Si el pedido fue ofrecido a otro conductor específico en el radar, no mostrarlo a este
    if (o.conductorOfrecidoId && o.conductorOfrecidoId !== driver.id) return false;
  ...
  // Solicitud entrante dirigida a este motorizado con temporizador de 15 segundos activo
  const activeIncomingOffer = orders.find(o => 
    o.conductorOfrecidoId === driver.id && 
    (o.tiempoRestanteAceptarConductor ?? 0) > 0 && 
    !o.conductoresRechazaron?.includes(driver.id) &&
    o.estado !== 'entregado' && 
    o.estado !== 'cancelado' && 
    !o.conductor
  );

  // Reproducir aviso sonoro y vibración cuando llega una nueva solicitud dirigida
  useEffect(() => {
    if (activeIncomingOffer) {
      playBellSound('driver');
      if (typeof navigator !== 'undefined' && 'vibrate' in navigator) {
        try { navigator.vibrate([200, 100, 200, 100, 400]); } catch {}
      }
    }
  }, [activeIncomingOffer?.id]);
  ...
  const handleDriverLogout = (e?: React.MouseEvent) => {
    ...
    return (
      <div className="flex flex-col h-full bg-neutral-100 dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100 p-4 items-center justify-center overflow-y-auto">
        <div className="w-full max-w-sm bg-white dark:bg-neutral-850 p-6 rounded-3xl border border-neutral-200 dark:border-neutral-800 shadow-xl space-y-4 my-auto">
          ...
          {/* Permisos de Dispositivo: Cámara, GPS y Archivos Multimedia */}
          <DevicePermissionsBanner appName="Vixy Delivery" />
          ...
          <div className="pt-2 border-t border-neutral-200 dark:border-neutral-800 space-y-2 text-center">
            <button
              type="button"
              onClick={() => setShowDriverRegModal(true)}
              className="w-full py-2.5 bg-amber-500/10 hover:bg-amber-500/20 text-amber-600 dark:text-amber-400 font-bold rounded-xl text-xs border border-amber-500/30 transition cursor-pointer flex items-center justify-center gap-1.5"
            >
            ...
            <button
              ...
              className="text-xs text-amber-600 dark:text-amber-400 hover:underline font-semibold cursor-pointer block mx-auto"
            >
  ...
  return (
    <div className="flex flex-col h-full bg-neutral-50 dark:bg-neutral-900 text-neutral-900 dark:text-neutral-100">
      ...
      <div className="p-3 bg-white dark:bg-neutral-850 border-b border-neutral-200 dark:border-neutral-800 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button
            ...
            onClick={(e) => {
              e.stopPropagation();
              handleDriverLogout(e);
            }}
            className="px-2 py-1.5 rounded-xl bg-red-500/10 hover:bg-red-500/20 text-red-600 dark:text-red-400 border border-red-500/30 transition cursor-pointer flex items-center gap-1 text-[10px] font-bold"
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
                  ...
                  <div className="space-y-3">
                    ...
                    {filteredOffers.map(order => {
                      ...
                      return (
                        <div
                          key={order.id}
                          className="p-4 bg-white dark:bg-neutral-850 rounded-2xl border-2 border-neutral-200 dark:border-neutral-750 shadow-sm space-y-3 hover:border-amber-500/40 transition relative overflow-hidden"
                        >
                          {/* 15-second acceptance timer banner */}
                          {order.tiempoRestanteAceptarConductor !== undefined && order.tiempoRestanteAceptarConductor > 0 && (
                            <div className="p-2.5 bg-amber-500/15 border border-amber-500/30 rounded-xl flex items-center justify-between">
                              <div className="flex items-center gap-2">
                                ...
                                <div>
                                  <p className="text-[11px] font-extrabold text-amber-700 dark:text-amber-300 flex items-center gap-1">
                                    <Bell className="w-3.5 h-3.5 text-amber-500 animate-bounce" />
                                    ¡Nuevo Servicio en tu Zona!
                                  </p>
                                  <p className="text-[9px] text-amber-600/90 dark:text-amber-400 font-medium">
                                    Tienes 15s para aceptar o saltará al siguiente conductor
                                  </p>
                                </div>
                              </div>
                              <div className="flex items-center gap-1.5 shrink-0">
                                <span className={`font-mono font-black text-xs px-2.5 py-1 rounded-lg shadow-xs ${
                                  order.tiempoRestanteAceptarConductor <= 5 
                                    ? 'bg-red-600 text-white animate-pulse' 
                                    : 'bg-amber-500 text-white'
                                }`}>
                                  00:{order.tiempoRestanteAceptarConductor < 10 ? '0' : ''}{order.tiempoRestanteAceptarConductor}s
                                </span>
                                <button
                                  type="button"
                                  onClick={() => playBellSound('driver')}
                                  title="Probar sonido de campana"
                                  className="p-1 text-amber-600 hover:text-amber-500 dark:text-amber-400 hover:bg-amber-500/10 rounded-lg cursor-pointer transition"
                                >
        ...
        {activeTab === 'cartera' && (
          <div className="space-y-3 text-xs">
            {/* Maintenance Warning if Cartera is paused from Web Panel */}
            {(!carterasConfig?.repartidoresHabilitada || !carterasConfig?.habilitadasGlobal) && (
              <div className="p-3 bg-red-500/10 border border-red-500/30 rounded-2xl flex items-start gap-2 text-red-700 dark:text-red-400 text-xs">
                <AlertTriangle className="w-4 h-4 shrink-0 text-red-500 mt-0.5" />
                <div>
                  <strong className="block">Cartera de Repartidores Pausada</strong>
                  <p className="text-[11px] opacity-90">
                    {carterasConfig.mensajeMantenimiento || 'La plataforma mantiene pausadas temporalmente las recargas y retiros de billeteras de delivery por mantenimiento.'}
                  </p>
                </div>
              </div>
            )}
      ...
      {showRechargeModal && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
          <div className="bg-white dark:bg-neutral-850 rounded-3xl border border-neutral-200 dark:border-neutral-800 p-4 sm:p-5 max-w-sm w-full max-h-[92dvh] overflow-y-auto shadow-2xl space-y-3.5 text-xs my-auto">
            <div className="flex items-center justify-between border-b border-neutral-100 dark:border-neutral-800 pb-2">
              <div>
                <h4 className="font-bold text-neutral-900 dark:text-white">
                  ...
                  <div className="space-y-1.5">
                    <div className="flex items-center justify-between">
                      <p className="font-bold text-emerald-500 flex items-center gap-1">
                        ...
                        Pago Móvil Oficial Vixy:
                      </p>
                      <button
                        type="button"
                        onClick={() => setShowPagoMovilModal(true)}
                        className="px-2 py-0.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded font-bold text-[9px] flex items-center gap-1 transition cursor-pointer"
                      >
                        <QrCode className="w-2.5 h-2.5" />
                        <span>Ver QR</span>
                      </button>
                    </div>
                    <p className="text-neutral-700 dark:text-neutral-300">Banco: <strong>{pagoMovilConfig.banco}</strong></p>
                    <p className="text-neutral-700 dark:text-neutral-300">Teléfono: <strong>{pagoMovilConfig.telefono}</strong></p>
                    <p className="text-neutral-700 dark:text-neutral-300">RIF / C.I.: <strong>{pagoMovilConfig.cedulaRif}</strong></p>
                    <p className="text-neutral-700 dark:text-neutral-300">Titular: <strong>{pagoMovilConfig.nombreTitular}</strong></p>
                    <p className="text-emerald-600 dark:text-emerald-400 font-bold text-[10px]">Monto en Bs: Bs. {(rechargeAmount * tasaBcv).toFixed(2)} (${rechargeAmount.toFixed(2)} USD)</p>
                  </div>
                )}
      ...
      {showIncidentModal && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
          <form onSubmit={handleCreateIncident} className="bg-white dark:bg-neutral-850 rounded-3xl border border-neutral-200 dark:border-neutral-800 p-4 sm:p-5 max-w-sm w-full max-h-[92dvh] overflow-y-auto shadow-2xl space-y-3 text-xs my-auto">
            <div className="flex items-center justify-between border-b border-neutral-100 dark:border-neutral-800 pb-2">
              <h4 className="font-bold text-neutral-900 dark:text-white">
                Reportar Incidencia en Tiempo Real
              </h4>
              <button type="button" onClick={() => setShowIncidentModal(false)} className="text-neutral-400 text-base cursor-pointer">×</button>
            </div>
      ...
      {rejectModalOrderId && (
        <div className="fixed inset-0 z-50 bg-black/60 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
          <div className="bg-white dark:bg-neutral-850 rounded-3xl border border-neutral-200 dark:border-neutral-800 p-4 sm:p-5 max-w-sm w-full max-h-[92dvh] overflow-y-auto shadow-2xl space-y-3 text-xs my-auto">
            <div className="flex items-center justify-between border-b border-neutral-100 dark:border-neutral-800 pb-2">
      ...
      {/* Modal / Alerta Flotante de Solicitud de Viaje Entrante (Temporizador de 15s) */}
      {activeIncomingOffer && (
        <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-xs flex items-center justify-center p-2.5 sm:p-4 overflow-y-auto animate-in fade-in duration-200">
          <div className="bg-white dark:bg-neutral-850 rounded-3xl border-2 border-purple-500 max-w-md w-full max-h-[92dvh] flex flex-col shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200 my-auto">
            {/* Top Bar with Urgency Meter */}
            <div className="bg-gradient-to-r from-purple-600 via-indigo-600 to-purple-700 text-white p-3 sm:p-4 flex items-center justify-between shrink-0">
              <div className="flex items-center gap-2">
                <div className="w-9 h-9 rounded-xl bg-white/20 flex items-center justify-center animate-bounce shrink-0">
                  <Bike className="w-5 h-5 text-white" />
                </div>
                <div className="min-w-0">
                  <span className="text-[9px] font-extrabold uppercase tracking-wider text-purple-200 block truncate">
                    ¡Solicitud de Delivery en tu Zona!
                  </span>
                  <h4 className="text-xs sm:text-sm font-black text-white truncate">
                    Pedido #{activeIncomingOffer.codigoSeguimiento}
                  </h4>
                </div>
              </div>

              {/* 15s Timer Ring / Badge */}
              <div className="flex items-center gap-1.5 bg-black/30 border border-white/20 px-2.5 py-1 rounded-xl shrink-0">
                <Clock className="w-3.5 h-3.5 text-amber-300 animate-spin" />
                <span className={`text-sm sm:text-base font-black font-mono ${
                  (activeIncomingOffer.tiempoRestanteAceptarConductor ?? 15) <= 5 ? 'text-red-300 animate-pulse' : 'text-white'
                }`}>
                  {activeIncomingOffer.tiempoRestanteAceptarConductor ?? 15}s
                </span>
              </div>
            </div>

            {/* Timer Progress Bar */}
            <div className="w-full bg-neutral-200 dark:bg-neutral-800 h-1.5 overflow-hidden shrink-0">
              <div 
                className={`h-full transition-all duration-1000 ${
                  (activeIncomingOffer.tiempoRestanteAceptarConductor ?? 15) <= 5 
                    ? 'bg-red-500' 
                    : (activeIncomingOffer.tiempoRestanteAceptarConductor ?? 15) <= 10 
                    ? 'bg-amber-500' 
                    : 'bg-purple-500'
                }`}
                style={{ width: `${Math.max(0, Math.min(100, ((activeIncomingOffer.tiempoRestanteAceptarConductor ?? 15) / 15) * 100))}%` }}
              />
            </div>

            {/* Scrollable Content Body */}
            <div className="p-3 sm:p-4 space-y-2.5 sm:space-y-3 text-xs overflow-y-auto flex-1 overscroll-contain">
              {/* Earnings highlight */}
              {(() => {
                const tripCost = calculateDeliveryTripCost(
                  activeIncomingOffer.metricasTiempo?.distanciaKm || 3.5
                );
                const gananciaNetoUsd = tripCost?.gananciaMotorizadoUsd ?? 2.5;
                const safeTasaBcv = tasaBcv || 36.5;
                const gananciaNetoBs = gananciaNetoUsd * safeTasaBcv;

                return (
                  <div className="p-2.5 sm:p-3 bg-emerald-500/10 border border-emerald-500/30 rounded-2xl flex items-center justify-between">
                    <div>
                      <span className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wider block">
                        Ganancia Neta para tu Cartera
                      </span>
                      <p className="text-lg sm:text-xl font-black text-emerald-600 dark:text-emerald-400 font-mono">
                        +${gananciaNetoUsd.toFixed(2)} USD
                      </p>
                    </div>
                    <div className="text-right">
                      <span className="text-[10px] text-neutral-400 block font-mono">
                        Tasa: {safeTasaBcv.toFixed(2)} Bs/$
                      </span>
                      <p className="text-xs font-bold text-neutral-700 dark:text-neutral-300 font-mono">
                        Bs. {gananciaNetoBs.toFixed(2)}
                      </p>
                    </div>
                  </div>
                );
              ...
              {/* Origin & Destination route steps */}
              <div className="space-y-1.5 sm:space-y-2 bg-neutral-50 dark:bg-neutral-900/60 p-2.5 sm:p-3 rounded-2xl border border-neutral-200 dark:border-neutral-800">
                <div className="flex items-start gap-2">
                  <div className="w-5 h-5 rounded-full bg-purple-500/15 text-purple-600 dark:text-purple-400 flex items-center justify-center shrink-0 mt-0.5">
                    <Store className="w-3 h-3" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <span className="text-[9px] font-bold text-neutral-400 uppercase block">1. Retiro en Comercio</span>
                    <p className="font-bold text-neutral-900 dark:text-white truncate text-xs">
                      {activeIncomingOffer.comercio?.nombre || 'Comercio Vixy'}
                    </p>
                    <p className="text-[10px] text-neutral-500 truncate">
                      {activeIncomingOffer.comercio?.direccion || 'Caracas'}
                    </p>
                  </div>
                </div>

                <div className="w-0.5 h-2 bg-neutral-300 dark:bg-neutral-700 ml-2.5" />
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    <span className="text-[9px] font-bold text-neutral-400 uppercase block">2. Entrega a Cliente</span>
                    <p className="font-bold text-neutral-900 dark:text-white truncate text-xs">
                      {activeIncomingOffer.detallesEntregaTienda?.contactoCliente || activeIncomingOffer.cliente?.nombre} {!activeIncomingOffer.esPedidoTienda && activeIncomingOffer.cliente?.apellido}
                    </p>
                    <p className="text-[10px] text-neutral-500 truncate">
                      {activeIncomingOffer.detallesEntregaTienda?.ubicacionEscrita || activeIncomingOffer.cliente?.direccion || 'Caracas'}
                    </p>
                  </div>
                </div>
              </div>
              ...
              <div className="p-2 sm:p-2.5 bg-neutral-100 dark:bg-neutral-800 rounded-xl flex items-center justify-between text-[10px] sm:text-[11px]">
                <span className="text-neutral-500">Modalidad de Cobro:</span>
                <span className="font-bold text-neutral-800 dark:text-neutral-200">
                  {activeIncomingOffer.metodoPagoSeleccionado === 'cartera_vixy' || activeIncomingOffer.detallesEntregaTienda?.modalidadCobro === 'ya_pagado_al_comercio'
                    ? '✅ Ya Pagado (No cobrar en puerta)'
                    : `💵 Cobrar en Efectivo: $${(activeIncomingOffer.montoTotalUsd ?? (activeIncomingOffer as any).total ?? 0).toFixed(2)}`}
                </span>
              </div>
            </div>

            {/* Pinned Action Buttons Footer */}
            <div className="p-3 bg-neutral-50 dark:bg-neutral-900 border-t border-neutral-200 dark:border-neutral-800 grid grid-cols-2 gap-2.5 shrink-0">
              <button
                type="button"
                onClick={() => {
                  setRejectModalOrderId(activeIncomingOffer.id);
                }}
                className="py-2.5 px-3 rounded-2xl bg-neutral-200 hover:bg-neutral-300 dark:bg-neutral-800 dark:hover:bg-neutral-700 text-neutral-800 dark:text-neutral-200 font-bold text-xs flex items-center justify-center gap-1.5 transition cursor-pointer"
              >
                <X className="w-4 h-4 text-red-500" />
                Rechazar
              </button>
              ...
              <button
                type="button"
                onClick={() => {
                  driverAcceptOrder(activeIncomingOffer.id);
                  setActiveTab('viajes');
                  setViajesSubView('activo');
                }}
                className="py-2.5 px-3 rounded-2xl bg-gradient-to-r from-emerald-600 to-teal-600 hover:from-emerald-500 hover:to-teal-500 text-white font-black text-xs flex items-center justify-center gap-1.5 shadow-lg shadow-emerald-600/30 transition cursor-pointer"
              >
                <CheckCircle2 className="w-4 h-4" />
                Aceptar ({activeIncomingOffer.tiempoRestanteAceptarConductor ?? 15}s)
              </button>
            </div>
          </div>
        </div>
      ...
      {/* Modal Emergente de Pago Móvil Oficial */}
      <PagoMovilModal
        isOpen={showPagoMovilModal}
        onClose={() => setShowPagoMovilModal(false)}
        montoUsd={rechargeAmount}
        concepto={`Recarga Cartera Conductor #${driver.id}`}
        initialReferencia={rechargeReference}
        onConfirmReference={(ref) => setRechargeReference(ref)}
      />
    </div>
  );
};
