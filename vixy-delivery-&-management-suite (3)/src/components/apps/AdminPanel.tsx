...
import { CompanyCommissionsManager } from '../admin/CompanyCommissionsManager';
...
export const AdminPanel: React.FC = () => {
  ...
  const allMenuItems = [
    ...
    { id: 'comisiones_empresa', label: 'Comisiones & Pago Móvil', icon: Percent },
  ...
  const allowedMenuItems = allMenuItems.filter(item => 
    ...
    currentAdminUser.nivelAcceso === 'super_admin' ||
    item.id === 'comisiones_empresa' ||
          ...
          <div className="flex items-center gap-2.5 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <span className="text-[10px] text-orange-400 uppercase font-mono block">
                {(currentAdminUser?.nivelAcceso || 'admin').replace('_', ' ')}
              </span>
            </div>
          </div>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-hidden bg-[#F1F5F9] dark:bg-slate-950">
        ...
        <header className="h-16 px-6 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between shrink-0 shadow-2xs">
          ...
          <div className="flex items-center gap-3">
            {/* Live 5-Second Sync Indicator */}
            <div className="hidden md:flex items-center gap-2 px-2.5 py-1.5 rounded-xl bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-xs">
              <span className="relative flex h-2 w-2">
                <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-emerald-400 opacity-75"></span>
                <span className="relative inline-flex rounded-full h-2 w-2 bg-emerald-500"></span>
              </span>
              <div className="flex flex-col text-[10px] leading-tight font-mono">
                <span className="font-semibold text-slate-700 dark:text-slate-200 flex items-center gap-1 font-sans">
                  Sync en Vivo (5s)
                  {isLiveSyncing && <span className="text-[9px] text-emerald-500 font-bold animate-pulse font-mono">...</span>}
                </span>
                <span className="text-slate-400 text-[9px]">
                  {lastSyncTime} • Próx: {syncCountdown}s
                </span>
              </div>
              <button
                onClick={triggerManualSync}
                disabled={isLiveSyncing}
                title="Forzar sincronización inmediata (5s)"
                className="p-1 hover:bg-slate-200 dark:hover:bg-slate-700 rounded-md text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 cursor-pointer transition"
              >
                <RefreshCw className={`w-3 h-3 ${isLiveSyncing ? 'animate-spin text-emerald-500' : ''}`} />
              </button>
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-neutral-100 dark:bg-neutral-950">
          ...
          {activeTab === 'comisiones_empresa' && <CompanyCommissionsManager />}
