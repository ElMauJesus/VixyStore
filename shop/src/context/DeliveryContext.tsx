...
interface DeliveryContextType {
  ...
  loginAdmin: (identifier?: string, password?: string, fallbackId?: string) => Promise<{ success: boolean; error?: string }> | { success: boolean; error?: string };
  logoutAdmin: () => void;
  ...
  sendChatMessage: (orderId: string, emisorTipo: 'cliente' | 'comercio' | 'conductor' | 'soporte', emisorNombre: string, mensaje: string) => void;
  ...
  openChat: (orderId: string, roleOrRecipient?: string) => void;
  closeChat: () => void;
...
export const DeliveryProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const [adminIsLoggedIn, setAdminIsLoggedIn] = useState<boolean>(() => {
    try {
      return !!localStorage.getItem('vixy_admin_session');
    } catch {
      return false;
    }
  });
  const [currentAdminUser, setCurrentAdminUser] = useState<AdminUser>(() => {
    try {
      const saved = localStorage.getItem('vixy_admin_session');
  ...
  const refreshBackendData = async () => {
    try {
      // 1. Configuración de tarifas y tasa BCV
      const cfg = await api.getConfig().catch(() => null);
      if (cfg?.success && cfg.config) {
        if (cfg.config.tasa_bcv) setTasaBcv(Number(cfg.config.tasa_bcv));
        setDeliveryRates(prev => ({
          ...prev,
          tasaBcvBs: Number(cfg.config.tasa_bcv || prev.tasaBcvBs),
          tarifaBaseMinimaUsd: Number(cfg.config.tarifa_base_usd || 2.00),
          distanciaBaseKm: Number(cfg.config.km_base || 3.0),
          costoPorFraccionUsd: Number(cfg.config.precio_km_adicional_usd || 0.50),
          porcentajeComisionDelivery: Number(cfg.config.comision_plataforma_porcentaje || 15)
        }));
      }
      ...
      if (storesRes?.success && Array.isArray(storesRes.comercios)) {
        ...
        // Si el comercio en sesión coincide con alguno cargado, sincronizarlo de forma segura
        setStore(curr => {
      ...
      // 3. Pedidos reales desde MySQL con Normalización Completa
      const pedRes = await api.getPedidos().catch(() => null);
      if (pedRes?.success && Array.isArray(pedRes.pedidos)) {
        const loadedOrders: Pedido[] = pedRes.pedidos.map((p: any) => {
          const storeObj: Comercio = (p.comercio && typeof p.comercio === 'object') ? p.comercio : {
            id: String(p.comercio_id || p.comercioId || ''),
            codigoComercio: String(p.comercio_id || ''),
            nombre: p.comercio_nombre || p.nombre_comercio || 'Comercio',
            nombreComercial: p.comercio_nombre || p.nombre_comercio || 'Comercio',
            rif: p.comercio_rif || '',
            categoria: 'General',
            categoriaPrincipal: 'comida_rapida',
            direccion: p.comercio_direccion || '',
            telefono: p.comercio_telefono || '',
            email: '',
            ...
            productos: [],
            categoriasCatalogo: [],
            metodosPagoAceptados: { pagoMovil: { activo: true }, carteraVixy: { activo: true }, efectivoUsd: { activo: true } },
            billetera: DEMO_COMERCIO_BILLETERA
          };

          const clientObj: Cliente = (p.cliente && typeof p.cliente === 'object') ? p.cliente : {
            ...
            username: '',
            direccion: p.destino_direccion || p.cliente_direccion || '',
            puntoReferencia: p.punto_referencia || '',
            email: '',
            avatarUrl: '/banners/banner_comercios.jpg',
            billetera: DEMO_CLIENTE_BILLETERA
          };

          let driverObj: Conductor | undefined = undefined;
          if (p.conductor && typeof p.conductor === 'object') {
            const c = p.conductor;
            driverObj = {
              ...c,
              ...
              legal: {
                cedula: c.legal?.cedula || c.cedula || '',
                licenciaGrado: c.legal?.licenciaGrado || '2da',
                licenciaNumero: c.legal?.licenciaNumero || c.licencia_numero || '',
                licenciaVencimiento: c.legal?.licenciaVencimiento || '2026-12-31',
                licenciaValida: c.legal?.licenciaValida !== undefined ? Boolean(c.legal.licenciaValida) : true,
                certificadoMedicoNumero: c.legal?.certificadoMedicoNumero || c.certificado_medico || '',
                certificadoMedicoVencimiento: c.legal?.certificadoMedicoVencimiento || '2026-12-31',
                certificadoMedicoValido: c.legal?.certificadoMedicoValido !== undefined ? Boolean(c.legal.certificadoMedicoValido) : true,
                rcvAseguradora: c.legal?.rcvAseguradora || 'Seguros Caracas',
                rcvPolizaNumero: c.legal?.rcvPolizaNumero || c.rcv_poliza || '',
                rcvVencimiento: c.legal?.rcvVencimiento || '2026-12-31',
                ...(typeof c.legal === 'object' ? c.legal : {})
              }
            };
          } else if (p.conductor_id) {
            driverObj = {
              id: String(p.conductor_id),
              nombre: p.conductor_nombre || 'Conductor',
              apellido: p.conductor_apellido || '',
              cedula: p.conductor_cedula || '',
          ...
          return {
            id: String(p.id),
            codigoSeguimiento: p.codigo_seguimiento || p.codigoSeguimiento || String(p.id),
            comercio: storeObj,
            cliente: clientObj,
            conductor: driverObj,
            ...
            montoSubtotalUsd: Number(p.monto_subtotal_usd || p.montoSubtotalUsd || 0),
            costoEnvioUsd: Number(p.costo_envio_usd || p.costoEnvioUsd || 0),
            distanciaKm: Number(p.distancia_km || p.distanciaKm || 3.0),
            metodoPago: p.metodo_pago || p.metodoPago || 'efectivo_usd',
            referenciaPago: p.referencia_pago || p.referenciaPago,
            pagoVerificado: Boolean(p.pago_verificado || p.pagoVerificado),
            solicitadoPor: p.solicitado_por || p.solicitadoPor || 'comercio',
            origenPedido: p.origen_pedido || p.origenPedido || 'tienda_independiente',
            esPedidoTienda: Boolean(p.es_pedido_tienda || p.esPedidoTienda),
            fechaCreacion: p.creado_en || p.fechaCreacion || new Date().toISOString(),
            comprobantePagoUrl: p.comprobante_pago_url || p.comprobantePagoUrl
          };
        });
      ...
      // 6. Reclamos reales
      const clmRes = await api.getReclamos().catch(() => null);
  ...
  const solicitarRecargaConductor = (montoUsd: number, metodoPago: MetodoPagoTipo, referencia: string, comprobanteUrl?: string): { success: boolean; error?: string } => {
    ...
    const nuevaSolicitud: SolicitudRecarga = {
      ...
      usuarioCedula: driver.legal?.cedula || driver.cedula || '',
      montoUsd,
      montoBs: Math.round(montoUsd * tasaBcv * 100) / 100,
      metodoPago,
      referencia,
  ...
  const updateClaimStatus = (claimId: string, nuevoEstado: 'en_espera_de_respuesta' | 'atendido' | 'solucionado', respuesta: string, autor: 'comercio' | 'backend' = 'backend') => {
    const timeStr = new Date().toISOString().replace('T', ' ').slice(0, 19);
  ...
  const updateStoreSchedule = (
    ...
    if (typeof scheduleData === 'string') {
      updates = { horarios: scheduleData, horarioApertura: scheduleData, abierto: abiertoParam ?? true };
    } else {
      updates = {
        ...scheduleData,
        ...
        horarioApertura: scheduleData.horarios || (scheduleData.horaApertura && scheduleData.horaCierre ? `${scheduleData.horaApertura} - ${scheduleData.horaCierre}` : undefined)
      };
    ...
    api.updateComercio(storeId, updates).catch(err => console.warn('Sync horario error:', err));
  ...
  const toggleStoreActive = (storeId: string) => {
    ...
    if (store.id === storeId) {
      setStore(prev => {
        const nAct = !prev.activo;
        return { ...prev, activo: nAct, abierto: nAct };
      });
    }
  ...
  // Auth Comercio Real (Valida contra MySQL y BD de registro)
  const logoutStore = () => {
    ...
    try {
      ...
      if (res && res.success && res.usuario) {
        ...
        const normalizedStore: Comercio = {
          id: String(u.id || u.codigoComercio || identifier),
          codigoComercio: u.codigoComercio || u.id || identifier,
          nombre: u.nombre || u.nombreComercial || 'Mi Comercio',
          nombreComercial: u.nombreComercial || u.nombre || 'Mi Comercio',
          rif: u.rif || identifier,
          categoria: u.categoria || 'Comercio General',
          categoriaPrincipal: u.categoriaPrincipal || 'comida_rapida',
          rubroPersonalizado: '',
          direccion: u.direccion || 'Caracas, Venezuela',
          telefono: u.telefono || '',
          email: u.email || '',
          logoUrl: u.logoUrl || '/banners/banner_comercios.jpg',
          bannerUrl: u.bannerUrl || '/banners/banner_comercios.jpg',
          calificacion: 5.0,
          totalCalificaciones: 0,
          tiempoEstimadoMin: 20,
          tiempoEstimadoMax: 45,
          costoEnvioUsd: 2.00,
          horarioApertura: u.horaApertura || '08:00 AM - 10:00 PM',
          horaApertura: u.horaApertura || '08:00:00',
          horaCierre: u.horaCierre || '22:00:00',
          diasOperacion: ['Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado'],
          activo: true,
          abierto: true,
          abiertoManual: true,
        ...
        try {
          localStorage.setItem('vixy_store_session', JSON.stringify(normalizedStore));
        } catch (e) {}

        addNotification('comercio', '🏪 Sesión Iniciada', `Panel comercial activo para ${normalizedStore.nombre}.`);
        return { success: true };
      } else {
  ...
  const loginAdmin = async (identifier?: string, password?: string, fallbackId?: string): Promise<{ success: boolean; error?: string }> => {
    ...
    if (identifier && password) {
      try {
        ...
        if (res?.success && (res.usuario || (res as any).user)) {
          ...
          try {
            localStorage.setItem('vixy_admin_session', JSON.stringify(userObj));
            if (res.token) localStorage.setItem('vixy_auth_token', res.token);
          } catch {}
          addActivityLog({
            usuarioId: userObj.id,
            usuarioNombre: userObj.nombre,
            usuarioRol: userObj.nivelAcceso,
            modulo: 'seguridad',
            accion: 'Inicio de Sesión en Panel Web (MySQL cPanel)',
            detalles: `Sesión autenticada en base de datos para ${userObj.nombre} (${userObj.username}).`,
            ip: '190.202.88.14 (Caracas, CANTV)',
            severidad: 'info'
          });
          return { success: true };
        ...
        // Continuar con verificación de respaldo
      }
    }

    // 2. Verificación local de respaldo
    ...
    setAdminIsLoggedIn(true);
    try {
      localStorage.setItem('vixy_admin_session', JSON.stringify(targetUser));
    } catch {}
  ...
  const logoutAdmin = () => {
    ...
    try {
      localStorage.removeItem('vixy_admin_session');
      localStorage.removeItem('vixy_auth_token');
    } catch {}
  ...
  const driverAcceptOrder = (orderId: string) => {
    ...
    addNotification('cliente', '🛵 Motorizado en Camino a la Tienda', `${driver.nombre} Ramírez (${driver.moto?.marca || 'Bera'} ${driver.moto?.placa || ''}) aceptó tu pedido #${acceptedCod} y se dirige al local.`);
    ...
    addActivityLog({
      ...
      detalles: `Motorizado aceptó pedido #${acceptedCod}. Se desplaza en ${driver.moto?.marca || 'moto'} ${driver.moto?.placa || ''} hacia el comercio.`,
        ...
        return {
          ...
          historialOperaciones: [
            ...o.historialOperaciones,
            {
              id: 'hist-' + Date.now(),
              estado: 'en_camino_al_comercio',
              descripcion: `Mesa operativa asignó al motorizado ${selectedDriver.nombre} ${selectedDriver.apellido} (${selectedDriver.moto?.marca || 'moto'} ${selectedDriver.moto?.placa || ''}). Motorizado notificado para retiro en ${comercioNombre}.`,
              actor: 'administrador',
              timestamp: timeStr
            }
          ]
        ...
        return {
          ...
          historialOperaciones: [
            ...o.historialOperaciones,
            {
              id: 'hist-' + Date.now(),
              estado: 'en_camino_al_cliente',
              descripcion: `Comercio verificó identidad del motorizado (${driver.nombre} ${driver.apellido}, Placa ${driver.moto?.placa || 'S/P'}) y entregó el paquete. Conductor en ruta hacia el cliente.`,
              actor: 'conductor',
              timestamp: timeStr
            }
          ]
  ...
  const openCall = (caller: string, callee?: string, phone?: string, role?: string) => {
    if (!callee && !phone && !role) {
      setCallModal({ isOpen: true, caller: 'Mesa Operativa', callee: caller, phone: '', role: 'usuario' });
    } else if (!role && phone && callee) {
      if (caller.startsWith('+') || caller.startsWith('04') || caller.includes('-') || !isNaN(Number(caller.replace(/\D/g, '')))) {
        setCallModal({ isOpen: true, caller: 'Mesa Operativa Vixy', callee: callee, phone: caller, role: phone });
      } else {
        setCallModal({ isOpen: true, caller: caller, callee: callee, phone: phone, role: 'usuario' });
      }
    } else {
      setCallModal({ isOpen: true, caller, callee: callee || '', phone: phone || '', role: role || 'usuario' });
    }
  };
  ...
  const openChat = (orderId: string, _roleOrRecipient?: string) => {
  ...
  const allDriverWallets = allDrivers.map(d => ({
    ...
    cedula: d.legal?.cedula || d.cedula || '',
