...
import { ZONAS_CALOR_CARACAS } from '../../data/initialData';
...
export const DeliveryRadarMap: React.FC<DeliveryRadarMapProps> = ({ 
  ...
  useEffect(() => {
    ...
    activeStores.forEach(com => {
      ...
      marker.on('click', () => {
        setSelectedComercio({ id: com.id, nombre: com.nombre, lat, lng, categoria: com.categoria });
      ...
      marker.bindTooltip(`
        <div style="padding: 2px 4px;">
          ...
          <span style="font-size: 10px; color: #94a3b8;">${com.categoria}</span>
        </div>
      `, { direction: 'top' });

      comerciosLayerRef.current?.addLayer(marker);
    });
  }, [showComercios, stores, onSelectComercio]);
        ...
        <div className="absolute bottom-3 left-3 bg-slate-900/95 backdrop-blur-md px-3 py-1.5 rounded-2xl border border-slate-800 text-[10px] text-slate-300 space-y-1 z-20 shadow-xl pointer-events-auto">
          <div className="flex items-center gap-3">
            ...
            <span className="flex items-center gap-1">
              ...
              Comercios ({stores.filter(s => s.activo !== false).length})
            </span>
            {showHeatmap && (
              <span className="flex items-center gap-1">
            ...
            <div className="grid grid-cols-2 gap-2 text-[10px]">
              <div className="p-2 bg-slate-800/80 rounded-xl space-y-0.5">
                ...
                <span className="font-bold text-slate-200 truncate block">
                  {selectedDriverOnMap.moto?.marca || 'Bera'} {selectedDriverOnMap.moto?.modelo || 'SBR'}
                </span>
                <span className="font-mono text-emerald-400 block">[{selectedDriverOnMap.moto?.placa || 'S/P'}]</span>
              </div>
              <div className="p-2 bg-slate-800/80 rounded-xl space-y-0.5">
