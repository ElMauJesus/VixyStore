              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-neutral-300 font-mono mt-0.5">
                    C.I: {inspectingDriver.legal?.cedula || inspectingDriver.cedula || 'N/A'} • Tel: {inspectingDriver.telefono}
                  </p>
                  <div className="flex items-center gap-2 text-xs text-amber-400 mt-1">
                    <Star className="w-3.5 h-3.5 fill-current" />
                    <span>{inspectingDriver.rating?.toFixed(1) || '5.0'} rating ({inspectingDriver.totalEntregas || 0} viajes)</span>
                    <span>•</span>
                    <span className="font-mono text-neutral-300">🛵 Placa: {inspectingDriver.moto?.placa || 'S/P'}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 bg-neutral-50 dark:bg-neutral-800/60 rounded-2xl border border-neutral-200 dark:border-neutral-700 space-y-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  <div className="p-2.5 bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-800">
                    ...
                    <span className="font-mono font-bold text-neutral-900 dark:text-white">Grado {inspectingDriver.legal?.licenciaGrado || '2da'} • {inspectingDriver.legal?.licenciaNumero || 'S/N'}</span>
                    <span className="text-[10px] text-emerald-500 block">Vigente hasta {inspectingDriver.legal?.licenciaVencimiento || '2026-12-31'}</span>
                  </div>

                  <div className="p-2.5 bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-800">
                    <span className="text-[10px] text-neutral-400 block font-bold uppercase">Certificado Médico MPPS</span>
                    <span className="font-mono font-bold text-neutral-900 dark:text-white">{inspectingDriver.legal?.certificadoMedicoNumero || 'S/N'}</span>
                    <span className="text-[10px] text-emerald-500 block">Vigente hasta {inspectingDriver.legal?.certificadoMedicoVencimiento || '2026-12-31'}</span>
                  </div>
                  ...
                  <div className="p-2.5 bg-white dark:bg-neutral-900 rounded-xl border border-neutral-200 dark:border-neutral-800">
                    ...
                    <span className="font-mono font-bold text-amber-600 dark:text-amber-400 block">Placa: {inspectingDriver.moto?.placa || 'S/P'}</span>
                    <span className="text-[10px] font-mono text-neutral-400 truncate block">Motor: {inspectingDriver.moto?.serialMotor || 'N/A'}</span>
                  </div>
                </div>
              </div>
