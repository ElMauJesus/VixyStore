            ...
            <div className="flex items-center gap-1.5 text-xs text-slate-600 dark:text-slate-400">
              ...
              {order.conductor ? (
                <span className="font-medium text-slate-800 dark:text-slate-200 truncate">
                  {order.conductor.nombre} ({order.conductor.moto?.placa || 'S/P'})
                </span>
              ) : (
          ...
          {orders.length === 0 ? (
            <>
              ...
              <p className="text-xs text-slate-400 max-w-xs mx-auto">
                Los pedidos generados por clientes y comercios aparecerán aquí en tiempo real una vez que la plataforma esté en operación.
              </p>
            </>
          ) : (
            <>
