...
import { DriverRegistrationRequestsManager } from './DriverRegistrationRequestsManager';
...
export const DriversManager: React.FC = () => {
  ...
  const [mainTab, setMainTab] = useState<'conductores' | 'solicitudes'>('conductores');
  const [inspectingSolicitud, setInspectingSolicitud] = useState<any | null>(null);
  ...
  return (
    <div className="space-y-6">
      ...
      {/* SECTION TABS: ACTIVE ROSTER VS REGISTRATION REQUESTS */}
      <div className="flex items-center gap-2 border-b border-neutral-200 dark:border-neutral-800 pb-2">
        <button
          onClick={() => setMainTab('conductores')}
        ...
        >
          ...
          <span>Padrón de Motorizados Activos ({totalDriversCount})</span>
        </button>

        <button
          onClick={() => setMainTab('solicitudes')}
          className={`px-4 py-2.5 rounded-xl text-xs font-bold transition cursor-pointer flex items-center gap-2 relative ${
            mainTab === 'solicitudes'
              ? 'bg-amber-500 text-white shadow-md'
              : 'bg-neutral-100 dark:bg-neutral-800 text-neutral-600 dark:text-neutral-400 hover:bg-neutral-200 dark:hover:bg-neutral-700'
          }`}
        >
          ...
          <span>Solicitudes de Registro Directo</span>
          {solicitudesRegistroConductores.filter(s => s.estado === 'pendiente_aprobacion').length > 0 ? (
            <span className="px-2 py-0.5 rounded-full text-[10px] font-mono font-black bg-red-500 text-white animate-pulse">
              {solicitudesRegistroConductores.filter(s => s.estado === 'pendiente_aprobacion').length} Pendientes
            </span>
          ) : (
            <span className="text-[10px] text-neutral-400 font-mono">
              ({solicitudesRegistroConductores.length})
            </span>
          )}
        </button>
      </div>
      ...
      {mainTab === 'solicitudes' ? (
        <DriverRegistrationRequestsManager />
      ) : (
        <>
